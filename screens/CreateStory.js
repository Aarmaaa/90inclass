...
export default class CreateStory extends Component {
  ...
  addStory = async() => {
    if(this.state.title && this.state.description && this.state.story && this.state.moral ){
      ...
      // var 
      await firebase.database().ref('/posts/' + Math.random().toString(36).slice(2)).set(storyData).then(function(data){
        //Send info to the feed screen on successful completion
        this.props.setUpdateToTrue( )
        this.props.navigation.navigate("Feed")
      })
    }
    else{
      Alert.alert("Fill in all the feilds",[{text:"ok", onPress:()=>console.log("ok has been pressed")}],{cancelable:false})
    }
  }
            ...
            <ScrollView>
              ...
              <View style={{ height: RFValue(this.state.dropdownHeight) }}>
                <DropDownPicker
                  ...
                  labelStyle={{
                    color: this.state.light_theme ? styles.DropDownLableLight : styles.dropDownLable,
              ...
              <View style = {{marginHorizontal: RFValue(10)}}>    
                <TextInput
                  style={this.state.light_theme ? styles.inputLightFont : styles.inputFont}
              ...
              <View>
                <Button title="Submit" color="#841584" onPress={()=>{
                  this.addStory()
                }} />
              </View>
